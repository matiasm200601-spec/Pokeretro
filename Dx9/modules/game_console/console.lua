...
consoleTextEdit = nil
chatInputToggle = nil
chatToggleUpdating = false
chatCloseInProgress = false
channels = nil
channelsWindow = nil
ignoreWindow = nil
ownPrivateName = nil
...
function init()
  ...
  chatInputToggle = consolePanel:getChildById('chatInputToggle')
  ...
  g_keyboard.bindKeyDown('Enter', openChatWithEnter, modules.game_interface.getRootPanel())
  g_keyboard.bindKeyDown('Enter', closeChatWithEnter, consolePanel)
...
function addPrivateText(text, speaktype, name, isPrivateCommand, creatureName)
  ...
  if privateTab == nil then
    ...
    privateTab.npcChannelName = 'NPCs'
  elseif focus then
...
function openChatWithEnter()
  if chatCloseInProgress or wasdChatActive then return end
...
function closeChatWithEnter()
  if not wasdChatActive then return end

  sendCurrentMessage()
  chatCloseInProgress = true
  setWASDChatMode(true)
  scheduleEvent(function() chatCloseInProgress = false end, 50)
end

function setWASDChatMode(enabled)
  wasdChatActive = false
  consoleTextEdit:disable()
  consoleTextEdit:hide()
  setChatToggle(false)
  modules.game_interface.getRootPanel():focus()
end

function setChatToggle(checked)
  if chatInputToggle and chatInputToggle:isChecked() ~= checked then
    chatToggleUpdating = true
    chatInputToggle:setChecked(checked)
    chatToggleUpdating = false
  end
end

function toggleChatInput(widget)
  if chatToggleUpdating then return end
...
function sendMessage(message, tab)
  ...
  if (channel or tab == defaultTab) and not chatCommandPrivateReady then
    ...
    elseif tab.npcChat then
      ...
      name = tab.npcChannelName or name
...
function online()
  ...
  scheduleEvent(function()
    if modules.client_options.getOption('wasdMovement') then
      setWASDChatMode(true)
    else
      setWASDChatMode(false)
    end
  end, 50)
end
